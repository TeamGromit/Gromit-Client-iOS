...
class SignInWithAppleDelegate: NSObject {
    ...
    private let signInSucceeded: (_ appleToken: String, _ email: String) -> Void
    private let siginInFail: () -> Void
    
    init(onSignedIn: @escaping (_ appleToken: String, _ email: String) -> Void, onFail: @escaping () -> Void) {
...
extension SignInWithAppleDelegate: ASAuthorizationControllerDelegate {
    ...
    func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
        switch authorization.credential {
            ...
            //            }
        case let appleIdCredential as ASAuthorizationAppleIDCredential:
            guard let toekn = getToken(credential: appleIdCredential) else {
                siginInFail()
                return
            }
            guard let tokenString = String(data: appleIdCredential.identityToken ?? Data(), encoding: .utf8) else {
                siginInFail()
                return
            }
            guard let email =  decode(tokenString: tokenString)["email"] as? String else {
                siginInFail()
                return
            }
            signInSucceeded(tokenString, email)
        default :
            siginInFail()
            break
        }
